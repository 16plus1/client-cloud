...
export const registration = async(email, password, password1) => {
    try {
        const response = await axios.post(`http://62.113.97.189:5000/api/auth/registration`, {
            email,
            password,
            password1
        })
...
export const login = (email, password) => {
    return async dispatch => {
        try {
            const response = await axios.post(`http://62.113.97.189:5000/api/auth/login`, {
                email,
                password
            })
...
export const auth = () => {
    return async dispatch => {
        try {
            const response = await axios.get(`http://62.113.97.189:5000/api/auth/auth`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } })
            dispatch(setUser(response.data.user))
            localStorage.setItem('token', response.data.token)
        } catch (e) {
            alert(e.response.data.message)
